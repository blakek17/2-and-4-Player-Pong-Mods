...
# Date: 15.11.16
# Description: Pong with a level framework based on points scored and 4 players
...
class Player():
        def __init__(self, properties, level):  
                ...
                if self.side == "left":
                        ...
                        self.x, self.y = SCR_WID/2, self.properties["offset"]
                        if level < 5: 
                                self.pad_wid, self.pad_hei = 64, 8
                        else:
                                self.pad_wid, self.pad_hei = 48, 8
                                
                else:
                        self.x, self.y = SCR_WID/2, SCR_HEI - self.properties["offset"]
                        if level < 5:
                                self.pad_wid, self.pad_hei = 64, 8
                        else:
                                self.pad_wid, self.pad_hei = 48, 8
        ...
        def scoring(self, scores):
                ...
                if self.side == "left":
                        ...
                        scores[2] = self.score
                        screen.blit(score_blit, (SCR_WID - (score_blit.get_width() + self.properties["offset"]), 16))
                elif self.side == "right":
                        scores[1] = self.score
                        screen.blit(score_blit, (SCR_WID - (score_blit.get_width() + self.properties["offset"]), SCR_HEI - (score_blit.get_height() + 16))) 
                else:
                        scores[3] = self.score
                        screen.blit(score_blit, (self.properties["offset"], SCR_HEI - (score_blit.get_height() + 16))) 
        ...
        def movement(self):
                ...
                if self.properties["name"] == "player 1" or self.properties["name"] == "player 2":
                        ...
                        if keys[self.properties["key_left"]]:
                                self.x -= self.speed
                        elif keys[self.properties["key_right"]]:
                                self.x += self.speed

                        if self.x <= 0:
                                self.x = 0
                        elif self.x >= SCR_WID - self.pad_wid:
                                self.x = SCR_WID - self.pad_wid
...
class Ball():
        ...
        def movement(self):
                ...
                for player in self.players:
                        ...
                        elif player.side == "top":
                                for n in range(-self.size, player.pad_wid):
                                        if self.x == player.x + n:
                                                if self.y <= player.y + player.pad_hei:
                                                        self.speed_y *= -1
...
def main():
        ...
        player_three_properties = {"name" : "player 3", "side" : "top", "offset" : 32, "key_left" : pygame.K_7, "key_right" : pygame.K_8}
        player_four_properties = {"name" : "player 4", "side" : "bottom", "offset" : 32, "key_left" : pygame.K_b, "key_right" : pygame.K_n}
        property_list = [player_one_properties, player_two_properties, player_three_properties, player_four_properties]
                                                 
        player_one = Player(player_one_properties, level)
        player_two = Player(player_two_properties, level)
        player_three = Player(player_three_properties, level)
        player_four = Player(player_four_properties, level)
